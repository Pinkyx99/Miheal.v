...
export const NotificationsDropdown: React.FC<NotificationsDropdownProps> = ({ show, onClose, hasUnclaimedBonus, onProfileUpdate }) => {
    ...
    const hasPromo = true; // This promotion is always available
    ...
    return (
        <div className="absolute top-full right-0 mt-2 w-80 bg-card border border-outline rounded-lg shadow-2xl z-40 p-4">
            ...
            { (hasUnclaimedBonus || hasPromo) ? (
                <div className="space-y-4">
                    {/* Real Money Promotion */}
                    {hasPromo && (
                        <div>
                            <div className="p-3 bg-background/50 rounded-lg">
                                <p className="font-semibold text-white">Play for Real Winnings!</p>
                                <p className="text-sm text-text-muted mt-1">Ready to turn your skills into real cash? Click here to play on our partner site.</p>
                                <a 
                                    href="https://gamdom.win/landing?aff=majkl"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="w-full mt-3 block text-center bg-primary hover:bg-primary-light text-white font-semibold py-2 rounded-md text-sm transition"
                                >
                                    Play Now
                                </a>
                            </div>
                    ...
                    {hasUnclaimedBonus && (
                        <div>
                            <div className="p-3 bg-background/50 rounded-lg">
                                <p className="font-semibold text-white">Welcome to Mihael.bet!</p>
