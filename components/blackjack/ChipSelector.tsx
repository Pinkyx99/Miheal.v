...
const Chip: React.FC<{ value: number; onClick: () => void; disabled: boolean }> = ({ value, onClick, disabled }) => {
    ...
    return (
        <button
            onClick={onClick}
            disabled={disabled}
            className={`w-10 h-10 sm:w-14 sm:h-14 rounded-full flex items-center justify-center font-bold text-white text-xs
                transition-transform duration-200 border-4 shadow-md
                disabled:opacity-50 disabled:cursor-not-allowed
                ${colorClasses}
                ${!disabled ? 'hover:scale-110 hover:-translate-y-1 sm:hover:-translate-y-2' : ''}
            `}
        >
...
export const ChipSelector: React.FC<ChipSelectorProps> = ({ onChipSelect, onClearBet, disabled }) => {
    return (
        <div className="flex justify-center h-[60px] sm:h-[100px] items-start pt-2">
             <div className="bg-black/40 backdrop-blur-sm p-2 sm:p-3 rounded-full flex items-center space-x-1 sm:space-x-3 border border-gray-600">
                <button 
                    onClick={onClearBet} 
                    disabled={disabled}
                    className="w-10 h-10 sm:w-14 sm:h-14 rounded-full flex items-center justify-center font-bold text-white text-sm bg-gray-700/80 border-4 border-gray-600 transition-transform duration-200 hover:scale-110 hover:-translate-y-1 sm:hover:-translate-y-2 disabled:opacity-50"
